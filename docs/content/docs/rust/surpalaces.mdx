...
`SurroundedPalaces` 持有的是四个宫位引用、不记语言，因此这里显式传——
通常传 `chart.language` 与本命盘保持一致；等价的自由函数是
...
**用途**　三方四正文本之后追加四宫每颗星的释义，每颗星只出现一次并标注所在宫。
...
| `lang` | `Language` | 是 | — | 输出语言，通常传 `chart.language` |

等价的自由函数是 `text::surrounded_palaces_to_text_with(sp, pack, lang)`。
