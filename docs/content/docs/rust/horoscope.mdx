...
**返回值**　`String`——分节的纯文本，各层带该层视角的格局行与流耀行。
...
println!("{}", h.to_text().chars().take(25).collect::<String>());
```
...
**用途**　`to_text` 的事实文本之后追加释义：各层流耀的释义、大限到流时各层视角命中格局的释义。
...
定义在 `HoroscopeRef` 上，按星盘排盘语言输出；要指定语言用自由函数
`text::horoscope_to_text_with(astrolabe, horoscope, pack, lang)`。
