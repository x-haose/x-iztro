...
| `is_leap_month` | `bool` | Yes | — | Lunar version only |
| `fix_leap` | `bool` | Yes | — | Whether to correct for leap months |
| `language` | `Language` | Yes | — | Output language |
...
in full, fetch the palace with `chart.palace(Palace::Soul)` and compare the `key` fields inside
`major_stars`.
</Accordion>
</Accordions>
