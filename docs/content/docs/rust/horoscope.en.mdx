...
The shared fields therefore go through `base`: write `h.yearly.base.heavenly_stem`, not
`h.yearly.heavenly_stem`. All four types are re-exported at the crate root.
