...
**Parameters** Same as `palace`; all four addressing forms work.

**Return value** `Option<FlankingPalaces<'_>>`, with two fields:

| Field | Offset | Type | Description |
| --- | --- | --- | --- |
| `previous` | -1 | `&PalaceData` | The preceding palace |
| `next` | +1 | `&PalaceData` | The following palace |
...
[surrounded set](/en/docs/rust/surpalaces); only the scope changes to these two palaces:

| Method | Meaning |
| --- | --- |
| `have(&[StarKey]) -> bool` | The two palaces together hold every star in the list |
| `not_have(&[StarKey]) -> bool` | Neither palace holds any of them |
| `have_one_of(&[StarKey]) -> bool` | The two together hold at least one |
| `have_mutagen(Mutagen) -> bool` | Either palace carries that natal mutagen |
| `not_have_mutagen(Mutagen) -> bool` | Neither one does |
...
println!("{} / {}", translate_palace(f.previous.name, en), translate_palace(f.next.name, en));
...
println!("{} / {}", translate_palace(w.previous.name, en), translate_palace(w.next.name, en));
...
[`has`](/en/docs/rust/palace#has--not_have--has_one_of) on `f.previous` or `f.next`.
</Accordion>
...
**Purpose** The chart's semantic text: a complete description for language models and people, as
a Markdown subset.
...
`PalaceRef::to_text()` / `SurroundedPalaces::to_text()`. The full format is on
...
[`TextOptions`](/en/docs/rust/astro#textoptions): pattern readings right after the pattern list,
each palace's star readings right after that palace's facts (same-palace major-star combinations
first), and `## Mutagen Notes` at the end. The factual lines are identical to `to_text`.
...
Emits in the charting language; for an explicit language use the free function
`text::astrolabe_to_text_with(astrolabe, opts, lang)`. Entry headings are translated into `lang`;
the bodies are the pack's text verbatim. Selection follows the same rule as
