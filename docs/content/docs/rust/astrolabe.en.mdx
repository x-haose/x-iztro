...
| Field | Type | Description |
| --- | --- | --- |
| `palaces` | `Vec<PalaceData>` | The twelve palaces; index 0 is the Yin palace, 11 the Chou palace |
...
println!("yearly  {} {}",
    translate_heavenly_stem(h.yearly.base.heavenly_stem, en),
    translate_earthly_branch(h.yearly.base.earthly_branch, en));
```

<Callout type="info">
`decadal` / `monthly` / `daily` / `hourly` are `HoroscopeItem`s whose pillars read directly, while
`yearly` and `age` each carry one extra datum of their own and keep the shared fields under `base` —
hence `h.yearly.base.heavenly_stem` rather than `h.yearly.heavenly_stem`.
</Callout>
