...
单宫与三方四正见 `PalaceRef::to_text()` / `SurroundedPalaces::to_text(lang)`。
完整格式见[语义化文本](/zh/docs/guide/guides/to-text)。

**示例**

```rust
println!("{}", chart.to_text().chars().take(46).collect::<String>());
```
...
**用途**　`to_text` 的事实文本之后追加按盘从知识包取出的释义：星耀释义（含同宫主星组合）、
格局释义、四化释义三节。
...
按排盘语言输出；要指定语言用自由函数 `text::astrolabe_to_text_with(astrolabe, pack, lang)`。
节标题与条目标题按 `lang` 翻译，正文是包里的原文。取材规则与
