...
| `is_leap_month` | `bool` | Yes | — | Whether that lunar month is a leap month. Has no effect if that month in that year has no leap month to begin with |
...
let a = by_lunar("2000-7-17", 2, Gender::Female, false, true, Language::EnUS, Config::default())?;
...
<Callout type="warn" title="The silent no-op of is_leap_month is deliberate">
