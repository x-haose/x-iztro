...
The two transit patterns (禄衰马困, 风云际会) never appear on a natal chart.
...
others) are judged at both, and `palace` records whichever matched — if both match, two hits come
back. 禄马交驰 goes further: it is reported for any palace that qualifies, so one chart may produce
several hits.
</Accordion>
...
`Scope::Decadal` view — `variant` `None` is decadal + minor limit, `Some("yearly")` is decadal +
annual, and if both hold, both are reported.
</Accordion>
