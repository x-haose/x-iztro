...
```rust
let chart = by_solar("1985-6-10", 8, Gender::Female, true, en, Config::default())?;
let cfg = PatternConfig {
    brightness_source: BrightnessSource::Positional,
    ..Default::default()
};
...
The pattern-hit list projects to text via `text::patterns_to_text`, one hit per line:
`- **pattern** (landing palace, broken): forming stars`, with only the palace in the parentheses
...
The form with readings follows the hit list with a reading for each hit pattern (`intro` plus a
`Conditions: ` line, each pattern once); the options are on
[TextOptions](/en/docs/rust/astro#textoptions):
