...
`astrolabe_to_text`）直接返回结果。错误类型见[错误处理](/zh/docs/rust/errors)。
</Callout>
...
`PalaceRef::to_text()`；`SurroundedPalaces::to_text(lang)` 收显式语言。
自由函数的 `lang` 可以与排盘语言不同：星名、时辰、星座、干支、流耀全部按标识
