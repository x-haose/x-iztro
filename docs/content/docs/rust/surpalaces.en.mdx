...
**Purpose** After the surrounded-palaces text, append a reading for each star in the four palaces;
each star appears once, tagged with its palace.
...
| `lang` | `Language` | yes | — | Output language, usually `chart.language` |

The equivalent free function is `text::surrounded_palaces_to_text_with(sp, pack, lang)`. The
section format is on [Text with readings](/en/docs/guide/guides/to-text#text-with-readings).
...
let sp = chart.surrounded_palaces(Palace::Soul).unwrap();
let text = sp.to_text_with(&pack, chart.language);

println!("{} {}", sp.to_text(chart.language).chars().count(), text.chars().count());
```
