...
| `YearlyStars` | 27 项，含 `xianchi` `huagai` `guchen` `guasu` `tiancai` `tianshou` `tianchu` `posui` `feilian` `longchi` `fengge` `tianku` `tianxu` `tianguan` `tianfu` `tiande` `yuede` `tiankong` `jielu` `kongwang` `xunkong` `tianshang` `tianshi` `jiekong` `jiesha` `nianjie` `dahao` |
...
| 模块 | 主要函数 |
| --- | --- |
| `star::location` | `get_start_index`、`get_lu_yang_tuo_ma_index`、`get_kui_yue_index`、`get_zuo_you_index`、`get_chang_qu_index`、`get_chang_qu_index_by_stem`、`get_daily_star_index`、`get_timely_star_index`、`get_kong_jie_index`、`get_huo_ling_index`、`get_luan_xi_index`、`get_huagai_xianchi_index`、`get_gu_gua_index`、`get_jiesha_adj_index`、`get_dahao_index`、`get_nianjie_index`、`get_tianshang_tianshi_index`、`get_tiancai_index`、`get_yearly_star_index`、`get_monthly_star_index` |
| `star::decorative` | `get_changsheng12`、`get_boshi12`、`get_yearly12`、以及上文两个起点函数 |
| `star::major` / `minor` / `adjective` | `get_major_stars`、`get_minor_stars`、`get_adjective_stars` |
