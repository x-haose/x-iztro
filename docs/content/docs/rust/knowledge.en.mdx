...
**Returns**　`Result<KnowledgePack, String>`. The error is a human-readable string, for two cases:
JSON that does not fit the format (`invalid knowledge pack: ...`), and a `schema` higher than
`SCHEMA_VERSION` (no best-effort downgrade).
...
println!("{} {:?}", pack.id, base.star_intro(StarKey::ZiweiMaj).map(|s| &s[..15]));
```
