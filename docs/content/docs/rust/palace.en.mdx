...
**Purpose** The palace's semantic text, byte-identical to that palace's section in the natal text:
a heading line with the stem-branch, decadal and Body/Original palace marks, then fact lines for
major stars (`Empty` for an empty palace), minor stars, adjective stars, trine & opposite, stem
flying, twelve gods and age-fortune years.
...
the free function `text::palace_to_text(&palace, lang)` (which takes a `&PalaceRef`).
...
**Purpose** Right after the palace's facts, a reading for each star in the palace (major, minor,
adjective), written `**star(brightness)[mutagen]**: body`, with same-palace major-star combinations
`**A × B (same palace)**: ` first; the twelve gods carry no readings.
...
Defined on `PalaceRef` and emitting in the chart's charting language; for an explicit language use
the free function `text::palace_to_text_with(&palace, opts, lang)`. Placement is on
