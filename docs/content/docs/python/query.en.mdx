...
`get_major_star_keys_by_solar_date` / `get_major_star_keys_by_lunar_date` below, or chart the whole
thing and compare the `key` of the `major_stars`.
</Accordion>
</Accordions>
...
**Return value** `list[str]` — keys from the `MajorStar` enum's domain (e.g. `"ziweiMaj"`); an empty
Soul palace borrows its opposite's major stars just the same. Keys are language-independent, so
these functions **take no `language`**.
