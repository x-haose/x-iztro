...
| `for_astrolabe(chart, config=None)` / `for_horoscope(horoscope, config=None)` | Pick by chart: a sub-pack holding only that chart's entries |

A `KnowledgePack` instance is also what the to_text family's `knowledge=` parameter accepts (see
each object's `to_text`).
...
| --- | --- | --- | --- | --- |
| `chart` | `Astrolabe` | yes | — | The chart to select by; a re-anchored chart selects by its re-anchored layout |
| `horoscope` | `Horoscope` | yes | — | A horoscope from `chart.horoscope(...)`; the natal part is selected along with it |
| `config` | `PatternConfig \| None` | no | `None` | Pattern criteria, the same shape as [`chart.patterns(config)`](/en/docs/python/patterns#patterns); omit for the default criteria |

**Returns**　A standard `KnowledgePack` with the metadata carried over: `stars` holds the stars on
the chart (major, minor, adjective and the four groups of twelve gods; the 14 major stars'
`combinations` keep only partners that actually share the palace), `patterns` the hits under the
`config` criteria, `mutagens` all four entries, `palaces` and `concepts` empty. `for_horoscope` adds
each scope's flowing stars and the patterns hit from the decadal down to the hourly perspective.
Pair the sub-pack with `patterns(config)` / `patterns_to_text(config)` under the same criteria:
different criteria hit different sets, and the readings would then miss or over-include entries.
The selection rule is on [Picking by chart](/en/docs/guide/guides/knowledge-pack#picking-by-chart).
...
only the Tianxiang entry. A horoscope constructed detached from a chart has no charting context and
`for_horoscope` raises `ValueError`.
