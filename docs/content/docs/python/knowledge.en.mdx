...
`Source`: `name`, `url`, `commit`, `license`, `author`, `retrieved_at`.
...
- Neither constructor **validates the format**: the version and structure are checked by the core
  during `merged`. In a pack you only read, an unrecognised field simply yields nothing.
