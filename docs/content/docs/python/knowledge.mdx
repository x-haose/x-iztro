...
| `horoscope` | `Horoscope` | 是 | — | 由 `chart.horoscope(...)` 得到的运限，本命部分随之取材 |
| `config` | `PatternConfig \| None` | 否 | `None` | 格局判定口径，与 [`chart.patterns(config)`](/zh/docs/python/patterns#patterns) 同形态；不传即默认口径 |
...
子包要和 `patterns(config)` / `patterns_to_text(config)` 配同一口径：口径不同，命中集合不同，
释义就会缺项或多项。
...
脱离星盘单独构造的运限没有排盘上下文，`for_horoscope` 抛 `ValueError`。
