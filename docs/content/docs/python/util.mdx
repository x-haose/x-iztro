...
**返回值**　`get_mutagen` 返回四化标识，该星不在此天干的四化表内时为 `None`。
`get_mutagens_by_heavenly_stem` 返回四项列表，顺序为**禄、权、科、忌**。
...
**返回值**　含 `soulIndex`、`bodyIndex`、`heavenlyStemOfSoul`、`earthlyBranchOfSoul` 的 dict，
后两项为语言无关标识。

**示例**

```python
print(utils.get_soul_and_body(6, 2, "gengHeavenly"))
```

**输出**

```text
{'bodyIndex': 8, 'earthlyBranchOfSoul': 'wuEarthly', 'heavenlyStemOfSoul': 'renHeavenly', 'soulIndex': 4}
```
...
**返回值**　十二项列表，**按宫位索引排列**——第 `i` 项就是 `chart.palaces[i]` 的宫名标识。

**示例**

```python
print(utils.get_palace_names(4)[:4])
```
...
**返回值**　含 `decadals` 与 `ages` 两项的 dict，均按宫位索引排列。
`decadals[i]` 含 `range` 岁数区间与该宫干支标识；`ages[i]` 是小限经过该宫的虚岁列表。
...
```text
{'earthlyBranch': 'yinEarthly', 'heavenlyStem': 'wuHeavenly', 'range': [43, 52]}
[9, 21, 33]
```
...
<Callout type="warn">
柱数不为四、某柱不是两项，或干支标识非法时抛 `ValueError`。
</Callout>
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `groups` | `list[list[Star]]` | 是 | — | 若干组十二宫星耀列表，每组长度须为 12 |
