...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | 否 | `None` | 释义材料：`True` 取排盘语言的内嵌默认包，`KnowledgePack` 用该包；给出时追加各层流耀的释义与大限到流时各层视角命中格局的释义，本命星耀不重复 |

**返回值**　`str`——按星盘排盘语言输出的分节纯文本，各层带该层视角的格局行与流耀行。
完整格式见[语义化文本](/zh/docs/guide/guides/to-text)，释义节格式见
...
print(h.to_text()[:25])
print(" ".join(l for l in h.to_text(knowledge=True).splitlines() if l.startswith("=== ")))
```
