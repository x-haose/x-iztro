...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | 否 | `None` | 释义材料：`True` 取排盘语言的内嵌默认包，`KnowledgePack` 用该包；给出时追加四宫每颗星的释义节（每星一次，标注所在宫），格式见[带释义的文本](/zh/docs/guide/guides/to-text#带释义的文本) |
