...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | no | `None` | Reading material: `True` takes the bundled default pack for the charting language, a `KnowledgePack` uses that pack; when given, a reading for each star in the palace (`**star(brightness)[mutagen]**: body`, same-palace major-star combinations first, twelve gods excluded) follows the palace's fact lines — see [Text with readings](/en/docs/guide/guides/to-text#text-with-readings) |
...
```python
print(chart.palace("soul").to_text(), end="")

zh = astro.by_solar("2000-8-16", 2, "female")          # the bundled pack is zh-CN
print(zh.palace("soul").to_text(knowledge=True).splitlines()[9][:15])
```
...
A palace constructed detached from a chart has no charting context and raises `ValueError`;
