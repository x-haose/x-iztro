...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | no | `None` | Reading material: `True` takes the bundled default pack for the charting language, a `KnowledgePack` uses that pack; when given, a note for each star in the palace follows the facts — format on [Text with readings](/en/docs/guide/guides/to-text#text-with-readings) |
...
print(zh.palace("soul").to_text(knowledge=True).splitlines()[11])
```
