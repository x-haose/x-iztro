...
要求 Python 3.10 及以上。发行版内含预编译的原生扩展，安装时不需要 Rust 工具链。
...
| `x_iztro.models` | `Astrolabe`、`Palace`、`Star`、`Horoscope` 等 dataclass | [星盘对象](/zh/docs/python/astrolabe) 起的四页 |
| `x_iztro.enums` | 全部语言无关标识的 StrEnum | [数据表](/zh/docs/python/data) |
...
# dataclasses.FrozenInstanceError: cannot assign to field 'solar_date'
```
