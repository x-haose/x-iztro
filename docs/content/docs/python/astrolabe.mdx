...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | 否 | `None` | 释义材料：`True` 取排盘语言的内嵌默认包，`KnowledgePack` 用该包（自定义或合并后的包）；给出时在事实之后追加星耀释义（含同宫主星组合）、格局释义、四化释义三节 |

**返回值**　`str`——按排盘语言输出的分节纯文本：基本信息、十二宫、格局命中。
