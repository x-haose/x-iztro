...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | no | `None` | Reading material: `True` takes the bundled default pack for the charting language, a `KnowledgePack` uses that pack; when given, notes on each scope's flowing stars and on the patterns hit from the decadal down to the hourly perspective follow the facts — natal stars are not repeated |
...
[Semantic text](/en/docs/guide/guides/to-text); the reading sections are on
...
print(" ".join(l for l in zh.to_text(knowledge=True).splitlines() if l.startswith("=== ")))
```
