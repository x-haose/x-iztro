...
**用途**　格局命中的语义化文本，每条一行 `- **格局名** (宫名, 破格): 构成星耀`，
破格在括号里与宫名并列。
...
内嵌默认包，`KnowledgePack` 用该包；给出时命中列表之后紧跟每条命中格局的释义
（`**格局名**: ` 正文加 `成立条件: ` 段，同一格局只出一次），见
...
print(chart.patterns_to_text(knowledge=True).splitlines()[2][:20])
```
