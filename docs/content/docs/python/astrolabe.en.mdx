...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | no | `None` | Reading material: `True` takes the bundled default pack for the charting language, a `KnowledgePack` uses that pack (custom or merged); when given, three sections follow the facts — star notes (with same-palace major-star combinations), pattern notes, mutagen notes |
...
the reading sections are on [Text with readings](/en/docs/guide/guides/to-text#text-with-readings).
