...
**Return value** `FlankingPalaces | None`, holding two `Palace` fields. Returns `None` when the palace
cannot be located.
...
`astrolabe()` gives back the chart the pair belongs to.

The five predicates are named and defined exactly as on the
[surrounded set](/en/docs/python/surpalaces); only the scope changes to these two palaces:

| Method | Meaning |
| --- | --- |
| `have(stars: list[str]) -> bool` | The two palaces together hold every star in the list |
| `not_have(stars: list[str]) -> bool` | Neither palace holds any of them |
| `have_one_of(stars: list[str]) -> bool` | The two together hold at least one |
| `have_mutagen(mutagen: Mutagen) -> bool` | Either palace carries that natal mutagen |
| `not_have_mutagen(mutagen: Mutagen) -> bool` | Neither one does |

Stars may be written as `MajorStar` / `MinorStar` enum members or as star names in the chart's language.
...
so `have_one_of` is `False`. Wealth sits in yin, flanked by health (Tiantong) and children (Taiyang);
...
[`has`](/en/docs/python/palace#has--not_have--has_one_of) on `f.previous` or `f.next`.
</Accordion>
...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | no | `None` | Reading material: `True` takes the bundled default pack for the charting language, a `KnowledgePack` uses that pack (custom or merged); when given, readings are inlined after the facts — pattern readings (with their conditions) follow the pattern list, each palace's star readings follow that palace's facts (same-palace major-star combinations first), and a `## Mutagen Notes` section closes the text |
...
**Return value** `str` — a Markdown subset in the charting language: a `# Natal Chart …` title,
`## Basic Info`, `## Palace Overview` (a table), `## Patterns`, `## Palaces` (one `### ` section
per palace from the Soul palace on). The full format is on
...
text = zh.to_text(knowledge=True)
print(len(zh.to_text()), len(text))
...
<Callout type="warn">
`knowledge=True` on a chart whose language has no bundled pack (only zh-CN has one) raises
`IztroError` (`invalid_argument`) rather than silently dropping the readings; on an English chart
pass a `KnowledgePack` explicitly.
</Callout>
