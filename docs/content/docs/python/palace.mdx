...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | 否 | `None` | 释义材料：`True` 取排盘语言的内嵌默认包，`KnowledgePack` 用该包；给出时追加该宫每颗星的释义节，格式见[带释义的文本](/zh/docs/guide/guides/to-text#带释义的文本) |

**示例**

```python
print(chart.palace("命宫").to_text())
print(chart.palace("命宫").to_text(knowledge=True).splitlines()[11])
```
