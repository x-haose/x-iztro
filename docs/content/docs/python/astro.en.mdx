...
description: The charting methods of the Astro class, re-anchoring, and the semantic text projection.
---
...
`chart.rearranged(...)`, `chart.horoscope(...)` and the to_text projection still use those two tables; they
...
**Return value** `str` — a Markdown subset (`#` headings, `- label: value` lists, a narrow
palace-overview table) in the chart's own charting language; the natal text carries a patterns
section and the twelve palaces in detail from the Soul palace on, and each horoscope scope carries
its mutagen, flowing-star and pattern lines from its own perspective. Every `to_text` accepts
`knowledge=` (`True` or a `KnowledgePack`); when given, readings picked by the chart are inlined
after the facts. Each also accepts `config=` (a `PatternConfig`), the criteria for the pattern
section and the pattern readings. The format is on [Semantic text](/en/docs/guide/guides/to-text).
