...
`KnowledgePack` uses that pack; when given, a reading for each hit pattern (`intro` plus a
"Conditions" line) follows the hit list — format on
...
print("\n".join(zh.patterns_to_text(knowledge=True).splitlines()[:5]))
```
