...
others) are judged at both, and `palace_index` records whichever matched — if both match, two hits
...
`Scope.DECADAL` view — `variant` `None` is decadal + minor limit, `"yearly"` is decadal + annual,
