...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `knowledge` | `bool \| KnowledgePack \| None` | no | `None` | Reading material: `True` takes the bundled default pack for the charting language, a `KnowledgePack` uses that pack; when given, a note for each star in the four palaces follows the facts (each star once, tagged with its palace) — format on [Text with readings](/en/docs/guide/guides/to-text#text-with-readings) |
