...
| `mutagenKeys` | string[4] | The same as keys |
...
The translated field is for people, the key field is for code. On stars the key field is simply
`key` (there is no `nameKey`); everywhere else it is "original field name + Key", and array-valued
ones use the plural `Keys` (`mutagenKeys`, `palaceNameKeys`, `yearlyKeys`).
