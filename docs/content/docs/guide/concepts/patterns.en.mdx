...
<Callout title="The two readings differ in exactly two placements">
x-iztro's brightness table matches iztro **value for value**, which is a hard line for the whole
library. It differs from the traditional position rule in exactly two placements:

- **Sun in You**: the table says Ping — neither bright nor dim; the position rule calls it dim
- **Moon in Yin**: the table says Wang, i.e. bright; the position rule treats it as neutral

So the two readings can only disagree on the **"both dim"** patterns (Sun and Moon Both Dim, Sun and
Moon Hiding Their Light). The **"both bright"** ones (Sun and Moon Both Bright, Cinnabar and Cassia
Steps) can never disagree: star placement geometry puts the Sun in Zi whenever the Moon is in Yin,
and the table calls that Xian, so neither reading forms the pattern.
</Callout>

```python
from x_iztro import Astro, PatternConfig, BrightnessSource

chart = Astro().by_solar("1985-6-10", 8, "female", language="en-US")
