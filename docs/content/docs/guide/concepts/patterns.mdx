...
风云际会有两种取法：`variant` 为空是「大限 + 小限」，
为 `"yearly"` 是「大限 + 流年」（另有流派取这一种）。两者都成立就报两条，
上面那张盘正是如此。
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 君臣庆会 | `jun_chen_qing_hui` | 紫微或天府与辅佐诸星成四种会合之一（见 variant） | `zi_po_zuo_you_jia` 紫微破军守命、左辅右弼分夹；`zi_xiang_chang_qu_axis` 紫微天相守命、昌曲分居命迁；`tian_fu_ji_yin_tong_liang_jia` 天府守命、机阴与同梁分夹；`zi_zuo_you_tong_gong` 紫微与左辅右弼同守命宫 | 有（仅第四形式：三方四正见煞忌时标记；前三形式以无煞忌为成格前提） | 通用 |
...
| 7 | 机月同梁 | `ji_yue_tong_liang` | 命宫在寅或申，宫内为天同天梁或天机太阴（空宫借对宫主星） | — | — | 通用 |
...
| 13 | 日照雷门 | `ri_zhao_lei_men` | 太阳、天梁同守命宫且命宫在卯 | — | — | 通用 |
...
| 38 | 禄衰马困 | `lu_shuai_ma_kun` | 运限命宫三方四正内，禄存与空曜或耗曜同宫，同时天马与煞忌同宫 | — | — | **行运** |
...
| 42 | 禄马交驰 | `lu_ma_jiao_chi` | 禄存与天马同宫（任一宫成立即报，可有多个命中） | — | — | 通用 |
...
| 45 | 禄马佩印 | `lu_ma_pei_yin` | 命宫禄存、天马、天相三星同宫 | — | 有（命宫见空亡星） | 通用 |
| 46 | 两重华盖 | `liang_chong_hua_gai` | 命宫禄存与化禄双禄坐守，又见地空或地劫 | — | — | 通用 |
| 47 | 风云际会 | `feng_yun_ji_hui` | 大限与另一限的命宫三方四正各自见禄存、天马或化禄 | `yearly` 取大限 + 流年（无 variant 为大限 + 小限） | — | **行运**（只在大限视角） |
...
| 60 | 文星暗拱 | `wen_xing_an_gong` | 文昌、文曲分居两宫遥拱命宫 | `opposite` 分居命宫与迁移宫；`trine` 分居两三合宫 | — | 通用 |
| 61 | 权禄生逢 | `quan_lu_sheng_feng` | 化权星与化禄星同守命宫，且两星皆庙旺 | — | — | 通用 |
| 62 | 科明暗禄 | `ke_ming_an_lu` | 化科守命宫，命宫的暗合宫有禄存 | — | — | 通用 |
...
「夹」指两颗星分别落在目标宫的**前后邻宫**（索引 -1 与 +1），
与三方四正毫无重叠。所以昌曲夹命、文星朝命、文星暗拱三条互不相交：
夹归夹命，会照（含同宫）归朝命，遥拱归暗拱。
</Accordion>
