...
In Rust the common fields of age fortune and the yearly scope live under `.base`
(`h.yearly.base.heavenly_stem`), because each of those two also has fields of its own. The layer is
flattened when serialized to Python and Go, where you write `h.yearly.heavenly_stem` directly.
</Callout>
