...
x-iztro is a port of [iztro](https://github.com/SylarLong/iztro) v2.6.1. Public iztro APIs have
equivalents in all three bindings — Rust, Python and Go — with identical capability and a form
that suits each language. Four additions from v2.6.0 are not yet ported; see
...
Added in iztro v2.6.0; x-iztro intends to port them but has not yet:

| iztro | What it is |
| --- | --- |
| `astrolabe.flankingPalaces(indexOrName)` | The two palaces flanking a target palace, as an object with `have` / `notHave` / `haveOneOf` / `haveMutagen` / `notHaveMutagen` |
| `astrolabe.decadalList()` | The decadals in the order they start, with nominal-age and year ranges, scope stars and mutagens |
| `astrolabe.yearlyList(indexOrName)` | The years within one decadal |
| `astrolabe.monthlyList(year, fixLeap)` | The months of one year; with a leap month and `fixLeap` on, it splits into 14 entries |

`toJSON()` (added to every functional class in v2.6.0) is not in this list: `to_json` / the DTOs in
all three bindings are already its equivalent.
...
| The `calendar` module | Dead code as of iztro v2.5.8 — every live code path goes through the `lunar-lite` dependency instead, and the module was not in the package's root exports; v2.6.1 removed it from the package |
...
### Knowledge packs (no iztro equivalent)
...
### Reverse lookup (no iztro equivalent)
...
- **The semantic text projection (to_text)**: project an astrolabe, a horoscope, a palace or the
