...
iztro 的公开 API 在 Rust、Python、Go 三侧都有等价物，
三侧能力完全一致，形式各随语言习惯。v2.6.0 新增的四组尚未跟进，见下方[尚未跟进的](#尚未跟进的)。
...
iztro v2.6.0 新增，x-iztro 计划跟进但尚未实现：

| iztro | 是什么 |
| --- | --- |
| `astrolabe.flankingPalaces(indexOrName)` | 取某宫的夹宫（前后相邻两宫）对象，带 `have` / `notHave` / `haveOneOf` / `haveMutagen` / `notHaveMutagen` 分析方法 |
| `astrolabe.decadalList()` | 按起运先后排列的大限列表，含起止虚岁、起止年份、该限流曜与四化 |
| `astrolabe.yearlyList(indexOrName)` | 某个大限内的流年列表 |
| `astrolabe.monthlyList(year, fixLeap)` | 某流年的流月列表；有闰月且 `fixLeap` 为真时按前后半月拆成 14 项 |

`toJSON()`（v2.6.0 给所有功能类新增）不在此列：三侧的 `to_json` / DTO 已是等价物。
...
- **语义化文本投影（to_text）**：把星盘、运限、宫位或三方四正投影成自然语言文本，
