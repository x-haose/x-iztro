...
立春), when the month changes (the 1st or the solar term), and which day the
...
| Reading | Pillars |
| --- | --- |
| Default (year at lunar new year, month at the 1st) | 辛巳 庚寅 乙未 己卯 |
| `Exact` (year at 立春, month at solar terms) | 庚辰 己丑 乙未 己卯 |
...
| `fix_leap` | leap month correction, same meaning as the charting parameter |
...
(運魁, 流昌 and family) never appear on a natal chart, and passing one is an
error.
...
figures (Apple Silicon, release build): the 5-year feature lookup above takes a
few milliseconds, the same conditions over 1900–2100 about half a second, and a
200-year BaZi lookup about 0.2 seconds.
