...
3. **The non-Chinese vocabularies come from iztro's word list and are not guaranteed to be the
   rendering conventional among practitioners in that language.** A few English entries are not
   words at all (`considery`, `disastery`). Translations are for display; predicate on the key
   fields.
</Callout>
