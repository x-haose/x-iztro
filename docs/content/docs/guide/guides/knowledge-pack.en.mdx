...
| --- | --- | --- |
| `stars` | 107 | 14 major, 14 minor, 34 adjective and 45 decorative stars. Each carries the card attributes (yin-yang, five elements, dipper, chemistry, career, duty, aliases, element colour, energy colour) plus a body of text; the 14 major stars also carry readings for pairing with each other major star |
...
| `concepts` | 38 | Glossary and basic concepts (same palace, the palace in question, six-harmony branches, the surrounded set …) |

The content comes from the 学习 pages of
[iztro-docs](https://github.com/SylarLong/iztro-docs) (MIT License, by Sylar Long). The generator
pins the source commit, and the pack's `source` section records the origin, commit, licence and
author in full. Text fields are Markdown.
...
```json
{
  ...
  "patterns": {
    "zi_fu_tong_gong": {
      "name": "紫府同宫",
      "quotes": ["紫府同宫终身福厚。"],
      "conditions": "紫府同宫的详细介绍见 紫微天府。这两颗星只会在寅宫和申宫同宫。",
      "intro": "福厚不厚其实是不一定的……"
    }
  },
  "palaces": { "soulPalace": { "name": "命宫", "intro": "命宫是决定星盘主人属性的宫位……" } },
  "mutagens": { "sihuaLu": { "name": "化禄", "intro": "化禄星简称 `禄`……" } },
...
println!("{}", &pack.star_intro(StarKey::ZiweiMaj).unwrap()[..30]);
```
...
print(pack.star_intro(MajorStar.ZIWEI)[:10])
```
...
fmt.Println(string([]rune(pack.StarIntro(iztro.StarZiweiMaj))[:10]))
```
...
| --- | --- | --- |
| 贪狼 | Water | 甲 Wood (qi is Water) |
| 巨门 | yin Earth | 癸 Water and 己 Earth (holding Metal and Wood) |
...
Every text in the default pack comes from the 学习 pages of
[iztro-docs](https://github.com/SylarLong/iztro-docs), MIT License, by Sylar Long. The pack
protocol, the generator and the three-language API are x-iztro's own work.
