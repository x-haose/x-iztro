---
title: 扩展星盘
description: 把自己的分析规则挂到星盘上——三种语言各自的扩展点。
---
...
`star.key == "ziweiMaj"` 在任何输出语言下都成立；
`star.name == "紫微"` 只在中文盘上成立。

扩展方法里做判断请用 `*_key` / `*Key` 字段或内置判断方法，
展示时才取译名。这样同一条规则在六种语言的盘上结果一致。
...
三种语言实现同一个插件：取命宫主星（空宫借对宫），并读出五行局的局数。
...
三段代码在同一张盘上都返回 `紫微`，换成英文排盘则都返回 `emperor`。
