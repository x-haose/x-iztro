...
The last argument is a `*Config`; pass `nil` for the defaults. Chart from a lunar date with
`ByLunar`, which takes one extra argument, `isLeapMonth bool`, positioned after `gender` and before
`fixLeap`:

```go
iztro.ByLunar("2000-7-17", 2, "female", false, true, "en-US", nil)
```
