...
Chart from a lunar date with [`by_lunar`](/en/docs/rust/astro#by_lunar), which takes one extra
argument, `is_leap_month`, positioned after `gender` and before `fix_leap`:

```rust
by_lunar("2000-7-17", 2, Gender::Female, false, true, Language::EnUS, Config::default())?;
```
