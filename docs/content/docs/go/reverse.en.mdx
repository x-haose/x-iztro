...
The condition set of a feature lookup. Zero values mean "unconstrained": an
empty string leaves that condition unset, a zero `YearRange` takes
`[1900, 2100]`, and a `Limit` of 0 takes the core default (512). Every
condition is optional, but at least one must be given.
...
| `FixLeap` | `bool` | leap month correction, same meaning as the charting parameter |
