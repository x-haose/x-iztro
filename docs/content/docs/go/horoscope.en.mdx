...
**Purpose** After the factual text of `ToText`, append readings: notes on each scope's flowing stars
and on the patterns hit from the decadal down to the hourly perspective. Natal star readings are not
repeated here — they are in the chart's `ToTextWith`.
...
| --- | --- | --- |
| `knowledge` | [`Knowledge`](/en/docs/go/knowledge#knowledge--builtinknowledge--knowledgefrom) | Source of the readings: `BuiltinKnowledge()` takes the bundled default pack for the chart language, `KnowledgeFrom(pack)` uses the given pack; the zero value behaves like `ToText` |

The section format is on [Text with readings](/en/docs/guide/guides/to-text#text-with-readings).

**Example** (on a zh-CN chart, since the bundled pack is Chinese)

```go
h, _ := chart.Horoscope("2025-1-1", 0)
text, _ := h.ToTextWith(iztro.BuiltinKnowledge())
