...
<Accordions>
<Accordion title="年系星的年支按 HoroscopeDivide 取">
年系杂耀属流年神煞，取年支时用的是 `HoroscopeDivide` 而非 `YearDivide`。
两个配置不同时，年系星与主星、辅星可能基于不同的年支——这是刻意的流派区分。
</Accordion>
<Accordion title="红鸾天喜不在 YearlyStarIndex 里">
它们也属年系，但由 `GetLuanXiIndex` 单独给出。
</Accordion>
<Accordion title="JiekongIndex / JieshaAdjIndex / DahaoAdjIndex 是中州派专有">
