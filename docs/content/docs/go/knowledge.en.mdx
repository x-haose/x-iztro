...
`Name`, `URL`, `Commit`, `License`, `Author`, `RetrievedAt`, all `string`, empty when absent.
...
**Returns**　`(*KnowledgePack, error)`; invalid JSON returns an `ErrInvalidArgument`-class error.
It does **not** check the format version — `schema` is validated by the core during `Merged`.
...
nil overlay, or a pack whose `schema` is newer than this library supports all return an
