...
**Purpose**　The source of readings for the `ToTextWith` family (`ToTextWith` / `PalaceToTextWith` /
...
| `Knowledge{}` (zero value) | No readings; the output is byte-identical to the method without `With` |
...
text, _ := chart.ToTextWith(iztro.KnowledgeFrom(mine))
```
