...
**返回值**　`string`——按星盘排盘语言输出的分节纯文本，各层带该层视角的格局行与流耀行。
...
fmt.Println(string([]rune(text)[:25]))
```
...
**用途**　`ToText` 的事实文本之后追加释义：各层流耀的释义、大限到流时各层视角命中格局的释义。
本命星耀的释义不在这里重复，在星盘的 `ToTextWith` 里。
...
| 参数 | 类型 | 说明 |
| --- | --- | --- |
| `knowledge` | [`Knowledge`](/zh/docs/go/knowledge#knowledge--builtinknowledge--knowledgefrom) | 释义材料来源：`BuiltinKnowledge()` 取盘语言的内嵌默认包，`KnowledgeFrom(pack)` 用给定的包；零值等同 `ToText` |
