...
type PatternConfig struct {
    ...
    Borrow           bool   // 空宫是否借对宫主星参与判定
    FlowStars        bool   // 运限视角下流曜是否等同对应本命辅星
}

func DefaultPatternConfig() *PatternConfig
```

`DefaultPatternConfig()` 返回 `{table, true, true}`，即传 `nil` 时的行为。
...
<Callout type="warn" title="不要用零值 PatternConfig">
`&iztro.PatternConfig{}` 的 `Borrow` 与 `FlowStars` 都是 `false`，
与默认口径不同。改口径请从 `DefaultPatternConfig()` 起手：
...
`Variant` 为空串是大限 + 小限，`"yearly"` 是大限 + 流年，两者都成立就报两条。
</Accordion>
<Accordion title="流曜等同本命辅星">
默认口径下运禄/流禄当禄存看、运昌/流昌当文昌看，其余同理。
不想要这个行为，从 `DefaultPatternConfig()` 起手把 `FlowStars` 置为 `false`。
</Accordion>
