...
```go
_, err := iztro.BySolar("2000-2-30", 2, "female", true, "zh-CN", nil)
...
用 `GenderMale`、`LanguageZhCN` 这些常量传参可以在写错的当场被编译器与 IDE 挡下。
...
for _, r := range rows {
    chart, err := iztro.BySolar(r.Date, r.TimeIndex, r.Gender, true, "zh-CN", nil)
    if err != nil {
        var e *iztro.Error
        errors.As(err, &e)
