...
| `gender` | `string` | Yes | — | `"male"` or `"female"`. Sets the direction of the decadal scope and of the Changsheng and Boshi gods |
...
| `language` | `string` | Yes | — | Output language; affects every translated field. The `*Key` fields are unaffected |
...
**Parameters** Identical to `BySolar` apart from the following.
...
| `isLeapMonth` | `bool` | Yes | — | Whether that lunar month is a leap month. Has no effect if that month in that year has no leap month to begin with |
...
```go
a, _ := iztro.ByLunar("2000-7-17", 2, "female", false, true, "en-US", nil)
b, _ := iztro.BySolar("2000-8-16", 2, "female", true, "en-US", nil)
...
<Callout type="warn" title="The silent no-op of isLeapMonth">
Pass `true` when that month is not a leap month and the parameter is silently ignored rather than
returning an error. If you need strict validation, confirm the leap month exists for that year and
...
chart, _ := iztro.BySolar("2000-8-16", 2, "female", true, "en-US", cfg)
...
chart, err := iztro.BySolar("2000-8-16", 2, "female", true, "en-US", cfg)
if err != nil {
    log.Fatal(err)
}
...
```go
chart, _ := iztro.BySolar("2000-8-16", 2, "female", true, "en-US", nil)
