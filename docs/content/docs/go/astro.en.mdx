...
description: BySolar, ByLunar, Rearranged and the semantic text projection.
---
...
Each also has a `With` variant (`ToTextWith(opts TextOptions)` etc.): with a knowledge pack the
readings follow the facts they belong to — see
[ToTextWith](/en/docs/go/astrolabe#totextwith--palacetotextwith--surroundedpalacestotextwith).
...
**Return value** `string` — a Markdown subset (`#` headings, `- label: value` lists, one palace
overview table) in the chart's own charting language; the natal text carries a patterns section
and the twelve palaces in detail starting from the Soul palace, and each horoscope scope carries
its own mutagen, flowing-star and pattern lines. Every notation is listed under
[Format conventions](/en/docs/guide/guides/to-text#format-conventions).
