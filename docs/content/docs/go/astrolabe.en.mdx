...
**Purpose** After the factual text of the matching `ToText`, append readings picked from a knowledge
pack by chart: the chart gets three sections — star notes (with same-palace major-star
combinations), pattern notes, mutagen notes; a palace or the surrounded palaces get a note for each
star in that palace / those four palaces.
...
| `knowledge` | [`Knowledge`](/en/docs/go/knowledge#knowledge--builtinknowledge--knowledgefrom) | Source of the readings: `BuiltinKnowledge()` takes the bundled default pack for the chart language, `KnowledgeFrom(pack)` uses the given pack; the zero value behaves like the method without `With` |

Each has a `Context` variant. The section format is on
...
text, _ := chart.ToTextWith(iztro.BuiltinKnowledge())

fmt.Println(len([]rune(plain)), len([]rune(text)))
var heads []string
