...
description: The fields of Astrolabe, its lookup methods, and the surrounded and flanking palaces.
---
...
**Purpose** Get a palace's flanking pair: the two palaces immediately before and after it on the chart.

**Zi Wei meaning** Phrases like "Qing Yang and Tuo Luo flank Ji" or "Sun and Moon flank the Soul palace"
are about exactly this pair. Flanking and the surrounded set are two non-overlapping lines of evidence:
the surrounded set asks how one group of energies echoes itself, flanking asks what stands on either side.
...
| Parameter | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `target` | `PalaceTarget` | Yes | — | A non-empty `Key` addresses by palace-name key (`PalaceBody` / `PalaceOriginal` also accepted); otherwise `Index` is used |

**Return value** `*FlankingPalaces`, with two `*Palace` fields:

| Field | Offset | Description |
| --- | --- | --- |
| `Previous` | -1 | The preceding palace |
| `Next` | +1 | The following palace |

The twelve palaces form a ring — indices wrap modulo 12, so palace 0 is preceded by palace 11.
Both fields are palaces of this chart, so `Palace`'s flying-mutagen and surrounded-set queries work as usual.

The five predicates are named and defined exactly as on the
[surrounded set](/en/docs/go/surpalaces); only the scope changes to these two palaces:

| Method | Meaning |
| --- | --- |
| `Have(stars ...string) bool` | The two palaces together hold every star in the list |
| `NotHave(stars ...string) bool` | Neither palace holds any of them |
| `HaveOneOf(stars ...string) bool` | The two together hold at least one |
| `HaveMutagen(mutagenKey string) bool` | Either palace carries that natal mutagen |
| `NotHaveMutagen(mutagenKey string) bool` | Neither one does |

Stars are variadic and may be written as constants such as `StarZiweiMaj` or as star names in the
chart's language.
...
The Soul palace sits in woo, flanked by siblings (si) and parents (wei). Tianji sits in siblings and
Tuoluo in parents — different palaces, yet `Have` still holds. Huoxing sits in spouse, outside the pair,
so `HaveOneOf` is `false`. Wealth sits in yin, flanked by health (Tiantong) and children (Taiyang);
this chart's natal stem geng turns Taiyang to Lu and Tiantong to Ji, so both mutagen questions are `true`.

**Edge cases and pitfalls**

<Accordions>
<Accordion title="Predicates run on the union of both palaces">
`Have(A, B)` means "A and B both appear across these two palaces", not that they sit together in one.
To look at a single side, call the palace's
[`Has`](/en/docs/go/palace#has--nothave--hasoneof) on `f.Previous` or `f.Next`.
</Accordion>
<Accordion title="No palace is missing a side">
Indices wrap modulo 12, so every one of the twelve palaces has a complete pair.
</Accordion>
</Accordions>
