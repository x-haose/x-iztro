...
格局文本见 `PatternsToText`（[格局判定](/zh/docs/go/patterns)）。

---

## ToTextWith / PalaceToTextWith / SurroundedPalacesToTextWith

**用途**　对应的 `ToText` 事实文本之后追加按盘从知识包取出的释义：星盘追加星耀释义
（含同宫主星组合）、格局释义、四化释义三节；单宫与三方四正追加该宫 / 四宫每颗星的释义。
...
| `knowledge` | [`Knowledge`](/zh/docs/go/knowledge#knowledge--builtinknowledge--knowledgefrom) | 释义材料来源：`BuiltinKnowledge()` 取盘语言的内嵌默认包，`KnowledgeFrom(pack)` 用给定的包；零值等同不带 `With` 的方法 |

各有 `Context` 变体。节格式见[带释义的文本](/zh/docs/guide/guides/to-text#带释义的文本)。
