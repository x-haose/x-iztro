...
```go
_, err := iztro.BySolar("2000-2-30", 2, "female", true, "en-US", nil)
...
for _, date := range []string{"2000-13-1", "2000-2-30", "1500-1-1"} {
    if _, err := iztro.BySolar(date, 2, "female", true, "en-US", nil); err != nil {
        fmt.Println(err)
    }
}
```
...
long month, 29 in a short one). Passing `true` for `isLeapMonth` when that year and month have no leap
month is not an error; the parameter is silently ignored.
</Accordion>
</Accordions>
...
```go
_, err := iztro.BySolar("2000-8-16", 13, "female", true, "en-US", nil)
fmt.Println(err)
```
...
Passing the `GenderMale` and `LanguageZhCN` constants lets the compiler and the IDE stop a typo where
it is written.

**Example**

```go
_, err := iztro.BySolar("2000-8-16", 2, "x", true, "en-US", nil)
...
for _, r := range rows {
    chart, err := iztro.BySolar(r.Date, r.TimeIndex, r.Gender, true, "en-US", nil)
    if err != nil {
        var e *iztro.Error
        errors.As(err, &e)
...
```go
build := func(date string, ti uint8, gender string) (*iztro.Astrolabe, error) {
    chart, err := iztro.BySolar(date, ti, gender, true, "en-US", nil)
    if err != nil {
