...
type PatternConfig struct {
    ...
    Borrow           bool   // whether an empty palace borrows the opposite palace's majors
    FlowStars        bool   // whether flowing stars count as their natal counterparts
}

func DefaultPatternConfig() *PatternConfig
```

`DefaultPatternConfig()` returns `{table, true, true}`, which is what passing `nil` does.
...
<Callout type="warn" title="Do not use a zero-value PatternConfig">
`&iztro.PatternConfig{}` has `Borrow` and `FlowStars` both `false`, which is not the default
reading. Start from `DefaultPatternConfig()` instead:

```go
cfg := iztro.DefaultPatternConfig()
cfg.BrightnessSource = iztro.BrightnessSourcePositional
```
</Callout>
...
two transit patterns (禄衰马困, 风云际会) never appear on a natal chart.
...
| `BrightnessSource` is neither valid value | `iztro: invalid patternConfig: unknown variant ...` |
...
Most patterns form at the Soul palace, but "Body-or-Soul" patterns (武贪同行, 杀破狼, 石中隐玉 and
others) are judged at both, and `PalaceIndex` records whichever matched — if both match, two hits
come back. 禄马交驰 goes further: it is reported for any palace that qualifies, so one chart may
...
禄衰马困 is judged at whichever level the current view is (decadal view judges the decadal, yearly
view the year); 风云际会 compares two limits across levels, so it is judged once, in the
`ScopeDecadal` view — an empty `Variant` is decadal + minor limit, `"yearly"` is decadal + annual,
and if both hold, both are reported.
</Accordion>
...
To turn that off, start from `DefaultPatternConfig()` and set `FlowStars` to `false`.
</Accordion>
