...
The text opens with a `## <palace> Trine & Opposite` heading followed by four sections — the
target, its opposite, the wealth position and the career position — each headed with its role
(`### Target Palace · soul (ren woo) · Decadal 3-12`, `### Opposite Palace · surface (wu zi) · …`)
and with fact lines identical to that palace's section in the natal text; `SurroundedPalacesToTextWith` places each palace's star
readings right after its fact lines. `opts` is on
[TextOptions](/en/docs/go/knowledge#textoptions).
