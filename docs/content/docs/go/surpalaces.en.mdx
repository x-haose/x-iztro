...
`SurroundedPalacesToTextWith` appends a note for each star in the four palaces (each star once,
tagged with its palace) after the surrounded-palaces text; the `knowledge` values are on
[Knowledge](/en/docs/go/knowledge#knowledge--builtinknowledge--knowledgefrom).
