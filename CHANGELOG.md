...
- `for_astrolabe` / `for_horoscope` 的四化条目改为按 key 取，与星、格局同一口径；
  此前整段照抄，包里的非法键会混进子包。
- 文档站页脚的分隔线在文档版式下横穿侧栏：侧栏是绝对定位、占满视口高度，
  而页脚横跨整行，滚到底部时页脚的上边框正落在侧栏中部，两者又都是透明背景。
  分隔线改为按 `--fd-sidebar-width` 起始，窄屏与首页版式下回到最左。
- 文档站「童限」一节的表列到虚岁 8 并标「循环」：童限不存在循环——起运最晚的火六局
  虚岁 6 即入第一个大限，童限最多到虚岁 5，口诀里的「六官禄」取不到。表改到 5 为止。
