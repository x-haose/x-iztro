...
fn check_case(case: &Value, case_idx: usize, failures: &mut Vec<String>) {
    ...
    let case_label = format!(
        "case[{}] date={} ti={} g={}",
        case_idx, solar_date, time_index,
    ...
    if astrolabe.palaces.len() != exp_palaces.len() {
        failures.push(format!("{}: palace count expected={} actual={}", case_label, exp_palaces.len(), astrolabe.palaces.len()));
        return;
    }
    ...
    for (pi, exp_palace) in exp_palaces.iter().enumerate() {
        ...
        if act_stem != exp_stem {
            failures.push(format!("{}: stem expected={} actual={}", pl, exp_stem, act_stem));
        }
        ...
        if act_branch != exp_branch {
            failures.push(format!("{}: branch expected={} actual={}", pl, exp_branch, act_branch));
        }

        // Major stars
        let exp_majors = exp_palace["major_stars"].as_array().unwrap();
        if act_palace.major_stars.len() != exp_majors.len() {
            failures.push(format!("{}: major count expected={} actual={}", pl, exp_majors.len(), act_palace.major_stars.len()));
        } else {
            for (si, exp_star) in exp_majors.iter().enumerate() {
                ...
                if act_sn != exp_sn {
                    failures.push(format!("{}: major[{}] name expected={} actual={}", pl, si, exp_sn, act_sn));
                }
                let exp_br = json_str(&exp_star["brightness"]);
                let act_br = fmt_brightness(act_star.brightness);
                if act_br != exp_br {
                    failures.push(format!("{}: major[{}] {} bright expected={:?} actual={:?}", pl, si, exp_sn, exp_br, act_br));
                }
                let exp_mu = json_str(&exp_star["mutagen"]);
                let act_mu = fmt_mutagen(act_star.mutagen);
                if act_mu != exp_mu {
                    failures.push(format!("{}: major[{}] {} mutagen expected={:?} actual={:?}", pl, si, exp_sn, exp_mu, act_mu));
                }
            }
        }
        ...
        let act_minor_set: BTreeSet<String> = act_palace.minor_stars.iter()
            .map(|s| translate_star(s.key, LANG).to_string()).collect();
        let exp_minor_set: BTreeSet<String> = exp_minors.iter()
            .map(|s| s["name"].as_str().unwrap().to_string()).collect();
        if act_minor_set != exp_minor_set {
            failures.push(format!("{}: minor set expected={:?} actual={:?}", pl, exp_minor_set, act_minor_set));
        } else {
            for exp_star_val in exp_minors.iter() {
                let exp_sn = exp_star_val["name"].as_str().unwrap();
                if let Some(act_star) = act_palace.minor_stars.iter()
                    .find(|s| translate_star(s.key, LANG) == exp_sn)
                {
                    let exp_br = json_str(&exp_star_val["brightness"]);
                    let act_br = fmt_brightness(act_star.brightness);
                    if act_br != exp_br {
                        failures.push(format!("{}: minor {} bright expected={:?} actual={:?}", pl, exp_sn, exp_br, act_br));
                    }
                    let exp_mu = json_str(&exp_star_val["mutagen"]);
                    let act_mu = fmt_mutagen(act_star.mutagen);
                    if act_mu != exp_mu {
                        failures.push(format!("{}: minor {} mutagen expected={:?} actual={:?}", pl, exp_sn, exp_mu, act_mu));
                    }
                }
            }
        }
        ...
        let act_adj_set: BTreeSet<String> = act_palace.adjective_stars.iter()
            .map(|s| translate_star(s.key, LANG).to_string()).collect();
        let exp_adj_set: BTreeSet<String> = exp_adjs.iter()
            .map(|s| s["name"].as_str().unwrap().to_string()).collect();
        if act_adj_set != exp_adj_set {
            failures.push(format!("{}: adj set expected={:?} actual={:?}", pl, exp_adj_set, act_adj_set));
        }

        // 12-series stars
        let twelve_checks: Vec<(&str, &str, &str)> = vec![
            ("changsheng12", exp_palace["changsheng12"].as_str().unwrap(),
             translate_star(act_palace.changsheng12, LANG)),
            ("boshi12", exp_palace["boshi12"].as_str().unwrap(),
             translate_star(act_palace.boshi12, LANG)),
            ("jiangqian12", exp_palace["jiangqian12"].as_str().unwrap(),
             translate_star(act_palace.jiangqian12, LANG)),
            ("suiqian12", exp_palace["suiqian12"].as_str().unwrap(),
             translate_star(act_palace.suiqian12, LANG)),
        ];
        for (field, expected, actual) in &twelve_checks {
            if actual != expected {
                failures.push(format!("{}: {} expected={} actual={}", pl, field, expected, actual));
            }
        }
        ...
        if act_palace.decadal.range != (exp_start, exp_end) {
            failures.push(format!("{}: decadal_range expected=({},{}) actual=({},{})",
        ...
        if act_dstem != exp_dstem {
            failures.push(format!("{}: dec_stem expected={} actual={}", pl, exp_dstem, act_dstem));
        }
        ...
        if act_dbranch != exp_dbranch {
            failures.push(format!("{}: dec_branch expected={} actual={}", pl, exp_dbranch, act_dbranch));
        }

        // Ages
        let exp_ages: Vec<u32> = exp_palace["ages"].as_array().unwrap()
            .iter().map(|v| v.as_u64().unwrap() as u32).collect();
        if act_palace.ages != exp_ages {
            failures.push(format!("{}: ages expected={:?} actual={:?}", pl, exp_ages, act_palace.ages));
        }
