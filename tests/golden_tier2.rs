...
fn golden_tier2_compact() {
    ...
    assert!(!entries.is_empty(), "No tier2 year files found in {}", TIER2_DIR);
    ...
    'outer: for entry in &entries {
        ...
        for (ci, case) in cases.iter().enumerate() {
            ...
            if act_sb != case.sb {
                failures.push(format!("{}: sb expected={} actual={}", case_label, case.sb, act_sb));
            }

            // Body branch
            let act_bb = translate_earthly_branch(astrolabe.earthly_branch_of_body_palace, LANG).to_string();
            if act_bb != case.bb {
                failures.push(format!("{}: bb expected={} actual={}", case_label, case.bb, act_bb));
            }

            // Five elements class
            let act_fc = translate_five_elements_class(astrolabe.five_elements_class, LANG).to_string();
            if act_fc != case.fc {
                failures.push(format!("{}: fc expected={} actual={}", case_label, case.fc, act_fc));
            }

            // Soul star
            let act_ss = translate_star(astrolabe.soul, LANG).to_string();
            if act_ss != case.ss {
                failures.push(format!("{}: ss expected={} actual={}", case_label, case.ss, act_ss));
            }

            // Body star
            let act_bs = translate_star(astrolabe.body, LANG).to_string();
            if act_bs != case.bs {
                failures.push(format!("{}: bs expected={} actual={}", case_label, case.bs, act_bs));
            }

            // Palaces
            if astrolabe.palaces.len() != 12 {
                failures.push(format!("{}: palace count={}", case_label, astrolabe.palaces.len()));
                if failures.len() >= MAX_FAILURES { break 'outer; }
                continue;
            }
            ...
            for pi in 0..12 {
                ...
                if act_name != exp_name {
                    failures.push(format!("{} p[{}]: name expected={} actual={}", case_label, pi, exp_name, act_name));
                }

                // Major stars (as sets to handle order differences)
                let act_majors: BTreeSet<String> = act_palace.major_stars.iter()
                ...
                if act_majors != exp_majors {
                    failures.push(format!("{} p[{}]: majors expected={:?} actual={:?}", case_label, pi, exp_majors, act_majors));
                }

                // Minor stars (as sets)
                let act_minors: BTreeSet<String> = act_palace.minor_stars.iter()
                    .map(|s| translate_star(s.key, LANG).to_string())
                    .collect();
                let exp_minors: BTreeSet<String> = case.ns[pi].iter().cloned().collect();
                if act_minors != exp_minors {
                    failures.push(format!("{} p[{}]: minors expected={:?} actual={:?}", case_label, pi, exp_minors, act_minors));
                }
                ...
                if act_range != exp_range {
                    failures.push(format!("{} p[{}]: decadal expected={:?} actual={:?}", case_label, pi, exp_range, act_range));
                }
            }
